...
def replay_execution_session(
    spec: ExecutionGatewaySpec, *, storage_root: Path, paper_bridge_environment: PaperBridgeEnvironment, paper_orders: list[OrderRequest],
    market_events: Iterable[MarketEvent], adapter_id: str, event_time: datetime,
) -> ReplayResult:
    manifest_store = ExecutionSessionManifestStore(storage_root)
    event_store = ExecutionSessionEventStore(storage_root)
    environment = RunnerEnvironment(manifest_store=manifest_store, event_store=event_store, paper_bridge_environment=paper_bridge_environment)

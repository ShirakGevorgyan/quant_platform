...
def _fit(objective: ObjectiveType, features: pd.DataFrame, labels: pd.Series, *, seed: int, hyperparameters: dict | None = None):
    ...
    model = factory.create(
        hyperparameters=ModelHyperparameters(values=hyperparameters or {"num_boost_round": 20}),
...
class TestXGBoostFitPredict:
    ...
    def test_different_seed_changes_output_with_subsampling(self) -> None:
        """Compares raw `predict_proba` (continuous) rather than the
        thresholded hard `predict()` label -- with strongly separable
        synthetic data, a small seed-driven difference in the fitted
        boosters can easily leave every hard label unchanged even though
        the underlying models genuinely differ (visible in each fitted
        model's own serialized bytes); the continuous score is the more
        sensitive, correct place to detect that."""
        features = _features()
        labels = _clf_labels(features)
        hp = {"num_boost_round": 20, "subsample": 0.6, "colsample_bytree": 0.6}
        ...
        assert f1.booster_raw_base64 != f2.booster_raw_base64
    ...
    def test_feature_importance_includes_zero_for_unused_features(self) -> None:
        """`Booster.get_score` OMITS features never used in any split --
        this wrapper must still report EVERY declared feature, filling
        in 0.0 for ones the booster silently dropped."""
        rng = np.random.default_rng(0)
        n = 200
        features = pd.DataFrame({"useful": rng.normal(size=n), "useless": np.zeros(n)})
        labels = pd.Series((features["useful"] > 0).astype(int))
        schema = FeatureSchema(feature_names=("useful", "useless"))
        factory = xgb_wrapper.XGBoostModelFactory()
        model = factory.create(hyperparameters=ModelHyperparameters(values={"num_boost_round": 10}), feature_schema=schema, objective=ObjectiveType.BINARY_CLASSIFICATION)
        fitted = model.fit(features, labels, seeds=SeedConfiguration(master_seed=1))
        importance = fitted.feature_importance()
        assert set(importance) == {"useful", "useless"}
        assert importance["useless"] == 0.0

    def test_serialization_round_trip(self) -> None:
        features = _features()
        fitted = _fit(ObjectiveType.BINARY_CLASSIFICATION, features, _clf_labels(features), seed=1)
        data = xgb_wrapper.XGBoostModelSerializer().serialize(fitted)
        restored = xgb_wrapper.XGBoostModelDeserializer().deserialize(data, expected_metadata=fitted.metadata)
    ...
    def test_early_stopping_produces_a_best_iteration(self) -> None:
        features = _features()
        labels = _clf_labels(features)
        fitted = _fit(
            ObjectiveType.BINARY_CLASSIFICATION, features, labels, seed=1,
            hyperparameters={"num_boost_round": 200, "early_stopping_rounds": 5, "validation_fraction": 0.2},
        )
        assert fitted.best_iteration is not None
        assert 0 <= fitted.best_iteration < 200
        preds = fitted.predict(features.iloc[:10])
        assert not np.isnan(preds).any()

    def test_without_early_stopping_best_iteration_is_none(self) -> None:
        features = _features()
        fitted = _fit(ObjectiveType.REGRESSION, features, _reg_labels(features), seed=1)
        assert fitted.best_iteration is None
